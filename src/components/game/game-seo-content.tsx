...
interface VideoContent {
  ...
  interactionStatistic?: Array<{
    ...
    interactionType: "https://schema.org/WatchAction" | "https://schema.org/LikeAction" | "https://schema.org/CommentAction";
...
const GameSeoContent: React.FC<GameSeoContentProps> = ({ 
  ...
  const generateRatingJsonLd = () => {
    ...
    if (gameUrl) {
      ratingData.url = gameUrl;
    }
    if (ratingContent.ratingCount) {
      ratingData.aggregateRating.ratingCount = ratingContent.ratingCount.toString();
    }
    if (ratingContent.reviewCount) {
      ratingData.aggregateRating.reviewCount = ratingContent.reviewCount.toString();
    }
  ...
  useEffect(() => {
    ...
    // 清理之前添加的 JSON-LD 脚本
    const existingScripts = document.querySelectorAll('script[type="application/ld+json"][data-game-seo]');
    ...
    // 清理函数：组件卸载时移除脚本
    return () => {
      const scripts = document.querySelectorAll('script[type="application/ld+json"][data-game-seo]');
